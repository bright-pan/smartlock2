 ...
 * 2012-05-29     aozima       support MTD nor flash test.
 */
...
static rt_err_t _block_device_test(rt_device_t device)
{
    ...
    if( (device->flag & RT_DEVICE_FLAG_RDWR) == RT_DEVICE_FLAG_RDWR )
    {
        ...
        if( result != RT_EOK )
        {
            rt_kprintf("device : %s cmd "
                       "RT_DEVICE_CTRL_BLK_GETGEOME failed.\r\n");
            return result;
        }
        ...
        // step 4: continuous single sector speed test
        {
            ...
            else
            {
                ...
                rt_kprintf("\r\nwrite 200 sector from %d to %d, ",
                           tick_start,tick_end);
                calculate_speed_print( (geometry.bytes_per_sector
                                        * 200UL * RT_TICK_PER_SECOND)
                                       / rt_tick_after(tick_start, tick_end) );
                rt_kprintf("\r\n");
        ...
        // step 5: random single sector speed test
        {
            ...
            else
            {
                ...
                for(i=0; i<200; i++)
                {
                    sector += rt_device_write(device,
                                              (geometry.sector_count / 10)
                                              * (i%10) + (i%10),
                                              read_buffer, 1);
                    if((i != 0) && ((i%4) == 0) )
                    {
        ...
        /* step 6: multiple sector speed test */
        {
            ...
            for(sector=2; sector<256; sector=sector*2)
            {
                ...
                {
                    rt_kprintf("no memory for %d sector!"
                               "multiple sector speed test abort!\r\n",
                               sector);
                    break;
                }

                rt_memset(multiple_buffer,
                          sector,
                          geometry.bytes_per_sector * sector);
                rt_kprintf("write: ");
                tick_start = rt_tick_get();
                for(i=0; i<10; i++)
                {
...
#ifdef RT_USING_MTD
static rt_err_t _mtd_device_test(rt_device_t device)
{
    ...
    mtd = RT_MTD_DEVICE(device);
    rt_kprintf("MTD: page size %d, block size %d. ",
               mtd->page_size,
               mtd->block_size);
    rt_kprintf("start %d, end %d\r\n\r\n", mtd->block_start, mtd->block_end);

    ptr = rt_malloc(mtd->block_size);
    if(ptr == RT_NULL)
    {
    ...
    /* erase all block */
    rt_kprintf("[info] erase all block...\n");
    tick_start = rt_tick_get();
    for (index = mtd->block_start; index < mtd->block_end; index++)
    {
        result = rt_mtd_erase_block(mtd, index * mtd->block_size);
        if(result != RT_EOK)
        {
            rt_kprintf("[ERR] erase fail at block #%d!\r\n", index);
            goto __mtd_exit;
        }
    }
    tick_end = rt_tick_get();
    rt_kprintf("[info] erase %d block: %d ticks.\r\n",
               mtd->block_end - mtd->block_start,
               rt_tick_after(tick_start, tick_end));

    /* read back after erase all */
    rt_kprintf("[info] read back after erase all...\r\n");
    for (index = mtd->block_start; index < mtd->block_end; index ++)
    {
        size = rt_mtd_read(mtd, index * mtd->block_size, ptr, mtd->block_size);
        if(size != mtd->block_size)
        {
            rt_kprintf("[ERR] read fail at block #%d\r\n", index);
            result = RT_EIO;
            goto __mtd_exit;
        }

        for (offset = 0; offset < mtd->block_size; offset ++)
        {
            if (ptr[offset] != 0xff)
            {
                rt_kprintf("[ERR] %d:wrong data at %d\r\n", offset, index);
                result = RT_EIO;
                goto __mtd_exit;
            }
        }
    }
    rt_kprintf("[pass] all block erased.\r\n\r\n");

    /* prepare write data. */
    for (index = 0; index < mtd->block_size; index ++)
    {
        ptr[index] = (rt_uint8_t)index;
    }

    /* write all block. */
    rt_kprintf("[info] write all block...\r\n");
    tick_start = rt_tick_get();
    for (index = mtd->block_start; index < mtd->block_end; index ++)
    {
        ptr[0] = (rt_uint8_t)index;
        size = rt_mtd_write(mtd, index * mtd->block_size, ptr, mtd->block_size);
        if(size != mtd->block_size)
        {
            rt_kprintf("[ERR] write fail at block #%d\r\n", index);
            result = RT_EIO;
            goto __mtd_exit;
        }
    }
    tick_end = rt_tick_get();
    rt_kprintf("[pass] write %d block: %d ticks. ",
               mtd->block_end - mtd->block_start,
               rt_tick_after(tick_start, tick_end));
    calculate_speed_print((mtd->block_size *
                           (mtd->block_end - mtd->block_start))
                          * RT_TICK_PER_SECOND
                          / rt_tick_after(tick_start, tick_end));
    rt_kprintf("\r\n\r\n");

    /* verify data for write. */
    rt_kprintf("[info] read and verify data for write...\r\n");
    tick_start = rt_tick_get();
    for (index = mtd->block_start; index < mtd->block_end; index ++)
    {
        size = rt_mtd_read(mtd, index * mtd->block_size, ptr, mtd->block_size);
        ...
        offset = 0;
        if (ptr[offset] != (rt_uint8_t)index)
        {
            rt_kprintf("[ERR] %d:wrong data at %d\n", offset, index);
            result = RT_EIO;
            goto __mtd_exit;
        ...
        for (offset = 1; offset < mtd->block_size; offset ++)
        {
            if (ptr[offset] != (rt_uint8_t)offset)
            {
                rt_kprintf("[ERR] %d:wrong data at %d\n", offset, index);
                result = RT_EIO;
                goto __mtd_exit;
    ...
    tick_end = rt_tick_get();
    rt_kprintf("[pass] read and verify %d block: %d ticks. ",
               mtd->block_end - mtd->block_start,
               rt_tick_after(tick_start, tick_end));
    calculate_speed_print((mtd->block_size *
                           (mtd->block_end - mtd->block_start))
    ...
    /* read back all block for speed test. */
    rt_kprintf("[info] read back for speed test...\r\n");
    tick_start = rt_tick_get();
    for (index = mtd->block_start; index < mtd->block_end; index ++)
    {
        size = rt_mtd_read(mtd, index * mtd->block_size, ptr, mtd->block_size);
    ...
    tick_end = rt_tick_get();
    rt_kprintf("[pass] read %d block: %d ticks. ",
               mtd->block_end - mtd->block_start,
               rt_tick_after(tick_start, tick_end));
    calculate_speed_print((mtd->block_size *
                           (mtd->block_end - mtd->block_start))
...
int device_test(const char * device_name)
{
    ...
    if (!(device->flag & RT_DEVICE_FLAG_ACTIVATED))
    {
        ...
        if (result != RT_EOK)
        {
            rt_kprintf("To initialize device:%s failed. "
                       "The error code is %d\r\n",
                       device->parent.name,
                       result);
            return result;
    ...
    case RT_Device_Class_MTD :
        rt_kprintf("MTD device!\r\n");
        return _mtd_device_test(device);
#endif
    default:
