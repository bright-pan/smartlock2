...
/* speak device */
struct gpio_pwm_user_data speak_user_data =
{
    DEVICE_NAME_SPEAK,
    GPIOB,
    GPIO_Pin_0,
...
gpio_device speak_device;

int rt_hw_speak_register(void)
{
    gpio_device *pwm_device = &speak_device;
    struct gpio_pwm_user_data *pwm_user_data = &speak_user_data;
...
struct gpio_pwm_user_data logo_led_user_data =
{
    ...
    RCC_APB1Periph_TIM5,
    24000000,
    65535,//38KHZ 1/38000/(1/72000000)
    0,
    TIM_CounterMode_Up,
    /* timer oc */
    TIM_OCMode_PWM2,
...
void TIM3_IRQHandler(void)
{
    ...
    rt_interrupt_enter();
    /* tim3 cc3 isr */
    if (TIM_GetITStatus(speak_user_data.timx, TIM_IT_CC3) == SET)
    {
        TIM_ClearITPendingBit(speak_user_data.timx, TIM_IT_CC3);
        if (speak_user_data.tim_pulse_counts > 0)
        {
            speak_user_data.tim_pulse_counts--;
        }
    }
    /* tim3 update isr */
    if(TIM_GetITStatus(speak_user_data.timx, TIM_IT_Update) == SET)
    {
        TIM_ClearITPendingBit(speak_user_data.timx, TIM_IT_Update);
        if (speak_user_data.tim_pulse_counts == 0)
        {
...
void TIM4_IRQHandler(void)
{
    ...
    rt_interrupt_enter();
    /* cc2 isr */
    if (TIM_GetITStatus(motor1_user_data.timx, TIM_IT_CC2) == SET)
    {
        TIM_ClearITPendingBit(motor1_user_data.timx, TIM_IT_CC2);
    ...
    /* cc2 isr */
    if (TIM_GetITStatus(motor2_user_data.timx, TIM_IT_CC1) == SET)
    {
        TIM_ClearITPendingBit(motor2_user_data.timx, TIM_IT_CC1);
...
void TIM5_IRQHandler(void)
{
    ...
    rt_interrupt_enter();
    /* tim5 cc1 isr */
    if (TIM_GetITStatus(logo_led_user_data.timx, TIM_IT_CC1) == SET)
    {
        TIM_ClearITPendingBit(logo_led_user_data.timx, TIM_IT_CC1);
    ...
    /* tim5 update isr */
    if(TIM_GetITStatus(logo_led_user_data.timx, TIM_IT_Update) == SET)
    {
        TIM_ClearITPendingBit(logo_led_user_data.timx, TIM_IT_Update);
        if (logo_led_user_data.tim_pulse_counts == 0)
        {
...
INIT_DEVICE_EXPORT(rt_hw_speak_register);
/*
