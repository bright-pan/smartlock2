...
const uint16_t sms_content_gate_temperature[] = {
...
const uint16_t sms_content_battery_remain_5p[] = {
		...
		if (result == RT_EOK)
		{
			...
			while (alarm_telephone_counts < TELEPHONE_NUMBERS)
			{
				if (device_parameters.telephone_address[alarm_telephone_counts].flag)
				{
					...
					resend_counts = 0;
					while (resend_counts < SMS_RESEND_NUM)
					{
						send_result = sms_pdu_ucs_send(device_parameters.telephone_address[alarm_telephone_counts].address, "8613800755500", sms_ucs, sms_ucs_length);
						rt_kprintf("SMS send_result = %d\n",send_result);
						if (send_result == 1)
						{
							break;
						}
					...
					rt_kprintf("\nresend counts :%d\n", resend_counts);
					if (resend_counts >= SMS_RESEND_NUM)
					{
						rt_kprintf("\nsend sms failure!!!\n");
						// send failure
					}
