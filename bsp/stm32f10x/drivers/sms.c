...
#define SMS_THREAD_PRI  				(SMS_THREAD_PRI_IS) 		//¶ÌÐÅÏß³ÌÓÅÏÈ¼¶
#define SMS_RECV_MAIL_OUTTIME   ((RT_TICK_PER_SECOND)*1)//¶ÌÐÅÏß³Ì½ÓÊÕÓÊ¼þ³¬Ê±
...
#define	TP_TYPE_DEFAULT					INTERNATIONAL_ADDRESS_TYPE//å›½é™…åœ°å€;
#define	TP_PID_DEFAULT					0x00//æ™®é€šGSM åè®®,ç‚¹å¯¹ç‚¹æ–¹å¼;
#define	TP_DCS_DEFAULT					0X08//UCS2ç¼–ç æ–¹å¼;
#define	TP_VP_DEFAULT					0XC2//5åˆ†é’Ÿæœ‰æ•ˆæœŸé™;

typedef struct {

	uint8_t SMSC_Length;//è®¡ç®—æ–¹å¼ä¸åŒ;
...
typedef struct {
	...
	uint8_t TP_OA_Length;//è®¡ç®—æ–¹å¼ä¸åŒ;
...
typedef struct {
	...
	uint8_t TP_DA_Length;//è®¡ç®—æ–¹å¼ä¸åŒ;
...
typedef struct {
	...
	uint8_t First_Octet;
	TP_OA_TYPE TP_OA;//9å­—èŠ‚;
	uint8_t TP_PID;
	uint8_t TP_DCS;
	TP_SCTS_TYPE TP_SCTS;//7å­—èŠ‚;
	uint8_t TP_UDL;//ç”¨æˆ·é•¿åº¦å¿…é¡»å°äºŽ140 ä¸ªå­—èŠ‚;
	uint8_t TP_UD[140];
...
typedef struct {
	...
	uint8_t	First_Octet;
	uint8_t	TP_MR;
	TP_DA_TYPE	TP_DA;//9å­—èŠ‚;
	uint8_t	TP_PID;
	uint8_t	TP_DCS;
	uint8_t	TP_VP;// 1ä¸ªå­—èŠ‚;
	uint8_t	TP_UDL;//ç”¨æˆ·é•¿åº¦å¿…é¡»å°äºŽ140ä¸ªå­—èŠ‚;
	uint8_t	TP_UD[420];
...
/*
  ...
  æ™ºèƒ½é”ç”µæ± ç”µé‡è¿˜å‰©50%ï¼Œè¯·å°½å¿«å……ç”µ
  ...
  æ™ºèƒ½é”ç”µæ± ç”µé‡è¿˜å‰©20%ï¼Œè¯·æ³¨æ„å®‰å…¨å¹¶å°½å¿«å……ç”µ
  ...
  æ™ºèƒ½é”æ‘„åƒå¤´è¢«é®æŒ¡ï¼Œè¯·æ³¨æ„å®‰å…¨
  ...
  å°Šæ•¬çš„ç”¨æˆ·ï¼Œä½ çš„é—¨é’¥æ²¡æœ‰æ‹”å–ï¼Œè¯·æ³¨æ„å®‰å…¨
